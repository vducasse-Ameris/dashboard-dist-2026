...
function _updateAumLogrosQ(q, targets) {
  ...
  var bars = { deuda: 'q-deuda-bar',   inm: 'q-inm-bar',   inter: 'q-inter-bar',   notas: 'q-notas-bar' };
  var baseColor = { deuda: '#1B4B9B', inm: '#2E6BAF', inter: '#5A93CC', notas: '#A8C5E0' };
  ...
  ['deuda', 'inm', 'inter', 'notas'].forEach(function(k) {
    ...
    if (el) { el.textContent = disp.txt; el.style.color = disp.color; }
    // Barra: ancho = % logrado (cap 100), color verde si >=100, rojo si rescate
    if (bar) {
      var w = disp.neg ? 0 : Math.max(0, Math.min(100, disp.pct));
      bar.style.width = w + '%';
      bar.style.background = disp.neg ? '#B33A2E' : (disp.pct >= 100 ? '#0E7A4E' : baseColor[k]);
    }
  });
  if (totalEl) {
