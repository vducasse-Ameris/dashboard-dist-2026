...
function _crossTableHead(cols) {
  return '<thead><tr style="border-bottom:2px solid var(--border2);">' + cols.map(function(c) {
    return '<th style="padding:7px 12px;text-align:' + c[1] + ';font-size:10px;color:var(--text3);'
      + 'text-transform:uppercase;letter-spacing:.05em;font-weight:600;white-space:nowrap;">' + c[0] + '</th>';
  }).join('') + '</tr></thead>';
}
...
function renderCrossSaldoReun() {
  ...
  if (!saldo || !Object.keys(saldo).length || !Object.keys(reun).length) {
    el.innerHTML = '<div style="padding:14px;text-align:center;color:var(--text3);font-size:11px;">'
      + 'Carga el Excel <strong>Follow Up clientes</strong> y el archivo de <strong>resultados de metas</strong> para ver el cruce.</div>';
    return;
  }
  var rows = [];
  ...
  if (!rows.length) {
    el.innerHTML = '<div style="padding:14px;text-align:center;color:var(--text3);font-size:11px;">'
      + 'Ningún distribuidor está simultáneamente en saldos por corte y con reuniones registradas.</div>';
    return;
  }
  var corte = (res.saldoCorteLabel ? ' · corte ' + res.saldoCorteLabel : '');
  var body = rows.map(function(c, i) {
    return '<tr style="border-bottom:1px solid var(--border);">'
      + '<td style="padding:7px 12px;text-align:right;font-size:10px;color:var(--text3);font-family:monospace;">' + (i + 1) + '</td>'
      + '<td style="padding:7px 12px;font-weight:500;color:var(--text);">' + c.nombre + '</td>'
      + '<td style="padding:7px 12px;text-align:right;font-family:monospace;font-weight:600;color:var(--text);">' + _fmtUSD(c.saldo) + '</td>'
      + '<td style="padding:7px 12px;text-align:center;font-family:monospace;color:var(--text);">' + c.reun + '</td>'
      + '</tr>';
  }).join('');
  el.innerHTML = '<div style="overflow-x:auto;"><table style="width:100%;border-collapse:collapse;font-size:12px;">'
    + _crossTableHead([['#', 'right'], ['Distribuidor', 'left'], ['Saldo (USD)' + corte, 'right'], ['Reuniones', 'center']])
...
function renderCrossGastoReun(d) {
  ...
  if (!list.length) {
    el.innerHTML = '<div style="padding:14px;text-align:center;color:var(--text3);font-size:11px;">'
      + 'Carga el Excel <strong>Follow Up clientes</strong> para ver el gasto vs reuniones por distribuidor.</div>';
    return;
  }
  var body = list.map(function(c, i) {
    var cpr = c.gastoPorReun != null
      ? _fmtCLP(c.gastoPorReun)
      : '<span style="color:var(--text3);">—</span>';
    var reunCell = c.reun > 0
      ? c.reun
      : '<span style="color:#B33A2E;font-weight:600;">0</span>';
    return '<tr style="border-bottom:1px solid var(--border);">'
      + '<td style="padding:7px 12px;text-align:right;font-size:10px;color:var(--text3);font-family:monospace;">' + (i + 1) + '</td>'
      + '<td style="padding:7px 12px;font-weight:500;color:var(--text);">' + c.nombre + '</td>'
      + '<td style="padding:7px 12px;text-align:right;font-family:monospace;font-weight:600;color:var(--text);">' + _fmtCLP(c.gasto) + '</td>'
      + '<td style="padding:7px 12px;text-align:center;font-family:monospace;color:var(--text);">' + reunCell + '</td>'
      + '<td style="padding:7px 12px;text-align:right;font-family:monospace;color:var(--text2);">' + cpr + '</td>'
      + '</tr>';
  }).join('');
  el.innerHTML = '<div style="overflow-x:auto;"><table style="width:100%;border-collapse:collapse;font-size:12px;">'
    + _crossTableHead([['#', 'right'], ['Distribuidor', 'left'], ['Gasto', 'right'], ['Reuniones', 'center'], ['CLP / reunión', 'right']])
    + '<tbody>' + body + '</tbody></table></div>';
}
